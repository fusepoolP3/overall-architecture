...
A methodology to evaluate the effectiveness of a linkage rule is to create a reference set of entities checked manually for their matches and non matches and try different linkage rules in order to find the one (optimal) that returns all the matches and non matches in the reference set.
...
UC 2) A user wants to interlink entities in a container to entities in a public data  set
...
* Choose a tool that provides common algorithms and metrics, uses RDF data, configurable and allows extensions  
...
* Parallelization of compaisons (block partition and use a thread for each similarity function)
