...
then a reasoner can infer :John owl:sameAs :Johnny. That said we cannot always rely on such properties and it can happen that the values contain errors so that it is always better to decide about the identity of two entities on more than one single property.
